...
func (repo *sceneRepository) FindEndScenesByStoryID(storyID bson.ObjectID) ([]domain.Scene, error) {
	ctx, cancel := repository.NewRequestTimeoutContext()
	defer cancel()

	var scenes []domain.Scene

	cursor, err := repo.collection.Find(ctx, bson.M{"storyId": storyID, "type": "end"})
	if err != nil {
		return nil, err
	}
